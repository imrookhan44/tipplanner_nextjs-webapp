...
import { collection, getDocs, addDoc, doc, getDoc, query, where } from "firebase/firestore";
 ...
 async function userDays(req: any, res: any ) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      const queryUserDays = query(dayCollectionRef, where('owner', '==', req.body.email), where('draftMode', '==', false))
      const docs = await getDocs(queryUserDays)
      const docsArray: any[] = []
      docs.forEach((doc) => {
        const data = doc.data()
